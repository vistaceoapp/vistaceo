...
export type PlatformType = "google" | "youtube" | "instagram" | "facebook" | "tiktok" | "web";
...
export const PlatformReputationCard = ({ 
  ...
  // Compact variant for secondary platforms
  if (variant === "compact" && !data.connected) {
    return (
      <button
        onClick={onConnect}
        className={cn(
          "flex items-center gap-2 p-2 rounded-lg border border-dashed border-border/50 hover:border-primary/50 transition-all group",
          className
        )}
      >
        <div className="w-6 h-6 rounded-md bg-muted flex items-center justify-center opacity-50 group-hover:opacity-100 transition-opacity">
          {config.icon}
        </div>
    ...
    return (
      <div className={cn(
        ...
        <div className="flex items-center gap-1">
          {data.metrics.mainScore !== undefined && (
