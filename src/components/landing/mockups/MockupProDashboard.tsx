...
import { TrendingUp, CheckCircle2, Sparkles, ChevronDown, RefreshCw, Zap } from "lucide-react";
...
const businessData: Record<BusinessKey, {
  ...
  mexico: {
    ...
    certaintyPct: 85,
    dimensions: [
      { name: "Tráfico", score: 74, icon: "🚶" },
      { name: "Rentabilidad", score: 70, icon: "📈" },
      { name: "Equipo", score: 66, icon: "👥" },
      { name: "Finanzas", score: 62, icon: "💰" },
      { name: "Eficiencia", score: 65, icon: "⚡" },
      { name: "Crecimiento", score: 68, icon: "🚀" },
  ...
  marketing: {
    ...
    certaintyPct: 79,
    dimensions: [
      { name: "Tráfico", score: 55, icon: "🚶" },
      { name: "Rentabilidad", score: 62, icon: "📈" },
      { name: "Equipo", score: 68, icon: "👥" },
      { name: "Finanzas", score: 48, icon: "💰" },
      { name: "Eficiencia", score: 60, icon: "⚡" },
      { name: "Crecimiento", score: 42, icon: "🚀" },
      { name: "Reputación", score: 58, icon: "⭐" },
    ],
...
export const MockupProDashboard = forwardRef<HTMLDivElement, MockupProDashboardProps>(({ business = "argentina" }, ref) => {
  ...
  const mainStyle = getHealthStyle(data.healthScore);
  const weakestDim = data.dimensions.reduce((min, d) => d.score < min.score ? d : min);
  const weakestStyle = getHealthStyle(weakestDim.score);
  ...
  return (
    <div ref={ref} className="bg-card/95 backdrop-blur-xl rounded-2xl border border-border shadow-2xl overflow-hidden w-full">
      ...
      <div className={cn("h-1.5", mainStyle.bgColorSolid)} />
      ...
      <div className="p-3 sm:p-4 md:p-5">
        ...
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="flex items-center gap-2">
            ...
            {/* Certainty badge — neutral, not health-colored */}
            <span className="inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-medium bg-muted/50 text-muted-foreground border border-border">
        ...
        <div className="flex items-start gap-3 sm:gap-4">
          ...
          <motion.div
            ...
            className={cn(
              ...
              "border",
              mainStyle.bgColor, mainStyle.borderColor
            )}
          >
            <div className="flex items-baseline gap-0.5">
              <span className={cn("text-3xl sm:text-4xl font-bold", mainStyle.textColor)}>
                {data.healthScore}
              </span>
              {/* Trend arrow — green only for positive change */}
              {data.healthChange > 0 && (
            ...
            <span className={cn("mt-1 px-2 py-0.5 rounded-full border text-[10px] sm:text-xs font-medium",
              mainStyle.bgColor, mainStyle.borderColor, mainStyle.textColor
            )}>
              {mainStyle.label}
            </span>
          </motion.div>
          ...
          <div className="flex-1 space-y-2 sm:space-y-2.5 pt-1">
            {data.dimensions.map((dim, i) => {
              const dimStyle = getHealthStyle(dim.score);
              return (
                <motion.div
                  key={dim.name}
                ...
                >
                  ...
                  <div className="flex-1 h-2.5 sm:h-3 bg-secondary rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className={cn("h-full rounded-full", dimStyle.bgColorSolid)}
                    />
                  </div>
                  <span className={cn("text-[10px] sm:text-xs font-bold w-6 sm:w-7 text-right", dimStyle.textColor)}>
                    {dim.score}
                  </span>
                </motion.div>
              );
        ...
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-border">
          <div className={cn("p-2.5 sm:p-3 rounded-xl border", weakestStyle.bgColor, weakestStyle.borderColor)}>
            <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
              <Zap className={cn("w-3.5 h-3.5 sm:w-4 sm:h-4", weakestStyle.textColor)} />
              <span className={cn("text-[10px] sm:text-xs font-semibold", weakestStyle.textColor)}>
                Prioridad: {weakestDim.name}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-border/50 text-[9px] sm:text-[10px] text-muted-foreground">
          ...
          {/* Weekly improvement in green — this IS a positive trend */}
          <div className="flex items-center gap-1">
