...
const QUESTIONS_CACHE_KEYS = [
  'setupQuestionsCache_adaptive_v7',
  'setupQuestionsCache_easy_v6_short_personalized',
];
function readQuestionCatalog(): Record<string, any> {
  try {
    const raw = QUESTIONS_CACHE_KEYS.map((k) => localStorage.getItem(k)).find(Boolean);
    if (!raw) return {};
...
const AREA_TO_CATEGORY: Record<string, string> = {
  ...
  'A2_TURISMO': 'servicio_profesional',
  'A3_RETAIL': 'comercio',
  'A4_SALUD': 'salud',
  'A5_EDUCACION': 'educacion',
  'A6_B2B': 'b2b',
...
const SetupPage = () => {
  ...
  const ensureDraftBusiness = useCallback(async (): Promise<string | null> => {
    ...
    try {
      // Consultar la base directamente evita crear dos drafts cuando el contexto
      // todavía no recibió el primero o el componente se remonta entre pasos.
      const { data: persistedDrafts } = await supabase
        .from('businesses')
        .select('id')
        .eq('owner_id', user.id)
        ...
        .limit(1);
      const existing = persistedDrafts?.[0] ?? businesses?.find((b) => !b.setup_completed);
      ...
      case 'country':
        return (
          <SetupStepCountry
            ...
            onChange={(code) => setData(d => ({ ...d, countryCode: code as CountryCode }))}
          />
        );
      case 'identity':
