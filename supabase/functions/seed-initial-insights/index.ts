...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Todo el trabajo pesado corre en background: aunque el usuario cierre la
    // pestaña o el splash deje de esperar, el sembrado termina igual.
    const doWork = async () => {
    ...
    if ((missionCount || 0) < 1) {
      ...
      try {
        ...
        const { data: biz, error: bizErr } = await supabase
          .from("businesses")
          .select("name, category, country, address, instagram_handle, google_place_id, avg_ticket, avg_rating, settings, channel_mix, monthly_revenue_range")
          .eq("id", businessId)
          .maybeSingle();
        if (bizErr) console.warn("[seed-initial-insights] business fetch error:", bizErr.message);
        ...
        if (LOVABLE_API_KEY && biz) {
          ...
          const ctx = {
            ...
            pais: biz.country,
            ciudad: ((biz.settings as any)?.city) || (biz as any).address || null,
          ...
          const missionCtl = new AbortController();
          const missionTimer = setTimeout(() => missionCtl.abort(), 40000);
          const aiRes = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
            method: "POST",
            signal: missionCtl.signal,
            headers: {
          ...
          if (aiRes.ok) {
            ...
            let parsed: any = null;
            try {
              parsed = raw ? JSON.parse(String(raw).replace(/^```(?:json)?/i, "").replace(/```$/, "").trim()) : null;
            } catch (parseErr) {
              console.warn("[seed-initial-insights] mission JSON parse failed:", String(raw).slice(0, 300));
            }
            if (
              parsed?.title &&
              parsed?.description &&
              ...
              console.warn("[seed-initial-insights] mission payload incomplete:", JSON.stringify(parsed)?.slice(0, 300));
            }
          } else {
            console.warn("[seed-initial-insights] AI mission generation HTTP", aiRes.status, (await aiRes.text()).slice(0, 300));
          }
        } else {
          console.warn(`[seed-initial-insights] mission skipped: apiKey=${!!LOVABLE_API_KEY} biz=${!!biz}`);
        }
    ...
    console.log(
      `[seed-initial-insights] Finished. reasons=${JSON.stringify(dedupedReasons)}`,
    );
    };

    // No bloqueamos la respuesta: el trabajo sigue aunque el cliente se vaya.
    try {
      // @ts-ignore EdgeRuntime existe en Supabase Edge Functions
      EdgeRuntime.waitUntil(doWork().catch((e) => console.error("[seed-initial-insights] background error", e)));
    } catch {
      doWork().catch((e) => console.error("[seed-initial-insights] background error", e));
    }

    return new Response(
      JSON.stringify({ success: true, started: true, mode: "background" }),
