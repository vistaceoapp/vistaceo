...
serve(async (req) => {
  ...
  try {
    ...
    // Handle case where no Facebook Pages are found
    if (!pagesData.data || pagesData.data.length === 0) {
      console.log("No Facebook Pages found for this account");
      
      // Store a "error" integration to track the attempt
      await supabase
        .from("business_integrations")
        .upsert({
          business_id: businessId,
          integration_type: platform === "facebook" ? "facebook" : "instagram",
          status: "error",
          metadata: {
            ...
            resolution_steps: [
              "Crear una Página de Facebook para tu negocio",
              "Asegurarte de ser administrador de la página",
              "Para Instagram: vincular tu cuenta profesional a la página",
            ],
          },
      ...
      return Response.redirect(
        `${APP_URL}/app/analytics?oauth=error&provider=meta&reason=no_pages&platform=${platform}`
      );
    }
