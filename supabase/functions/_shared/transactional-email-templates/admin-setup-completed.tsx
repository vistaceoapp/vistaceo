...
const Email = ({ email, fullName, businessName, businessId, countryCode, areaId, userId, timestamp }: Props) => (
  <Html lang="es" dir="ltr">
    <Head />
    <Preview>Setup completado: {businessName || 'negocio'}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={badge}>
          <Text style={badgeText}>VISTACEO · NOTIFICACIÓN</Text>
        </Section>
        <Heading style={h1}>Setup completado · Usuario activo</Heading>
        ...
        <Row label="País" value={countryCode} />
        <Row label="Rubro" value={areaId} />
        <Row label="Business ID" value={businessId} />
...
export const template = {
  component: Email,
  subject: (d: Record<string, any>) => `✅ Setup completado: ${d.businessName ?? 'negocio'} (${d.email ?? '—'})`,
...
const badgeText = { margin: 0, color: '#34c759', fontSize: '11px', fontWeight: 600, letterSpacing: '0.4px' }
